...
export default function ProgressTracker({ notes, isVisible, onToggle, currentProject }) {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      const settingsList = await ProjectSettings.filter({ project_id: currentProject?.id || 'global' });
  ...
  const totalChars = projectNotes.reduce((sum, note) => sum + getCharCount(note.content), 0);
  // 프로젝트 전체 목표는 각 노트의 target_words를 합산하여 계산
  const projectWordTarget = projectNotes.reduce((sum, note) => sum + (note.target_words || 0), 0);

  const dailyProgress = Math.min((todayWrittenChars / settings.daily_word_target) * 100, 100);
  // 프로젝트 진행률은 totalChars를 projectWordTarget으로 나눕니다. 0으로 나누는 경우를 방지합니다.
  const projectProgress = projectWordTarget > 0 ? Math.min((totalChars / projectWordTarget) * 100, 100) : 0;
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <Target className="h-4 w-4" />
              {currentProject ? `${currentProject.title} 진행 상황` : "전체 진행 상황"}
            </CardTitle>
        ...
        <CardContent className="space-y-4">
          {isEditing ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2"> {/* 그리드 컬럼을 1개로 변경 */}
                <Input
                  type="number"
                  placeholder="일일 목표 (글자)"
                  value={tempSettings.daily_word_target || ''}
                  onChange={(e) => setTempSettings(prev => ({ ...prev, daily_word_target: Number(e.target.value) }))}
                />
                {/* 프로젝트 목표 글자수 입력 필드는 각 문서의 목표 글자수 합계로 자동 계산되므로 제거 */}
              </div>
              <p className="text-xs text-slate-500">프로젝트 전체 목표는 각 문서의 목표 글자수 합계로 자동 계산됩니다.</p>
              <div className="flex gap-2">
                <Button onClick={saveSettings} size="sm" className="flex-1">저장</Button>
                <Button onClick={() => { setIsEditing(false); setTempSettings(settings); }} variant="outline" size="sm" className="flex-1">취소</Button>
              </div>
            </div>
          ) : (
            <>
              <div>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono">
                    {todayWrittenChars.toLocaleString()} / {settings.daily_word_target.toLocaleString()} 글자
                  </span>
                </div>
                <Progress value={dailyProgress} className="mt-1" />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono">
                    {totalChars.toLocaleString()} / {projectWordTarget.toLocaleString()} 글자
                  </span>
                </div>
                <Progress value={projectProgress} className="mt-1" />
              </div>
