...
export const renderLinksInContent = (
  content,
  allNotes,
  ...
  return content.replace(
    ...
    (match, embedChar, linkText) => {
      ...
      // 임베드('!') 문자가 있는 경우
      if (embedChar === "!") {
        const embeddedContent = renderLinksInContent(
          targetNote.content || "",
          allNotes,
          targetNote.project_id,
          depth + 1
        );
        return `
...
const findBidirectionalLinks = (currentNote, allNotes, currentProjectId) => {
  ...
  const outbound = outboundMatches
    ...
    .map(({ isEmbed, linkText }) => {
      // isEmbed와 linkText를 구조 분해 할당
      let targetNote = null;
              ...
              <div className="space-y-2">
                {outbound.map((note, index) => (
                  ...
                  >
                    ...
                    {note.exists !== false &&
                      note.project_id &&
                      currentProject?.id !== note.project_id && (
                        <Badge
                          variant="outline"
                          className="text-xs"
                        >
