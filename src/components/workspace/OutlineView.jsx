...
const CollapsibleOutlineItem = ({
  ...
  const targetProgress = note.target_words
    ? Math.min((charCount / note.target_words) * 100, 100)
    : 0;
  ...
  const handleSave = async () => {
    try {
      await Note.update(note.id, {
        ...editData,
        target_words: editData.target_words
          ? Number(editData.target_words)
          : null,
      });
      setIsEditing(false);
      onUpdate();
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="revised_draft">
                              수정안
                            </SelectItem>
...
export default function OutlineView({
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      ...
      await Promise.all(
        updates.map(({ id, sort_order }) => Note.update(id, { sort_order }))
      );
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="gap-2"
            >
              <Plus className="h-4 w-4" />새 장/절 추가
            </Button>
          </div>
        </div>
