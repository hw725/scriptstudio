      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {footnotes.length === 0 ? (
          <div className="text-center text-slate-500 py-8">
            ...
            <p className="text-xs mt-2 text-slate-400">에디터 상단의 '각주 추가' 버튼으로 새 각주를 만드세요.</p>
          </div>
        ) : (
          footnotes.map(footnote => (
            <div key={footnote.id} className="flex gap-3 p-3 bg-white rounded-lg border border-slate-200/60">
              <div className="font-semibold text-slate-600 text-sm flex-shrink-0 w-8 pt-2">[{footnote.id}]</div>
