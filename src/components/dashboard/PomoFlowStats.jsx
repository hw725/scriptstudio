...
import { BrainCircuit, Coffee, PlayCircle, PauseCircle, Loader2 } from 'lucide-react';
...
const TimerDisplay = ({ state, linkedNote }) => {
    ...
    useEffect(() => {
        const calculateTimeLeft = () => {
            if (!state || state.status !== 'running' || !state.start_time || !state.duration_minutes) {
                setTimeLeft('');
                return;
            }
            const startTime = new Date(state.start_time);
            const endTime = new Date(startTime.getTime() + state.duration_minutes * 60000);
            const now = new Date();
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 capitalize">{state.session_type.replace('_', ' ')}</p>
                </div>
            </div>
...
export default function PomoFlowStats() {
    ...
    const linkedNote = overview?.timerState?.task?.id
        ? allNotes.find(note => note.pomoflow_task_id === overview.timerState.task.id)
        : null;
        ...
        return (
            <Card>
                ...
                <CardContent className="flex items-center justify-center py-10">
                    ...
                    <p className="ml-4 text-slate-600">PomoFlow 데이터를 불러오는 중...</p>
                </CardContent>
            </Card>
        );
    }
    ...
    return (
        <Card>
            ...
            <CardContent>
                <TimerDisplay state={overview.timerState} linkedNote={linkedNote} />
            </CardContent>
        </Card>
    );
}
