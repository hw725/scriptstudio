  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500">{getRelativeTime(item.updated_date)}</p>
      </div>
...
const ProjectCard = ({ project, stats, onSelect }) => {
  ...
  const progress = stats.targetChars > 0 ? Math.min((stats.currentChars / stats.targetChars) * 100, 100) : 0;
      ...
      <CardContent className="space-y-3">
        {stats.targetChars > 0 ? (
          ...
          <div className="py-2">
            ...
            <div className="text-lg font-semibold">{stats.currentChars.toLocaleString()} 글자</div>
            <div className="text-xs text-slate-500 mt-1">문서에 목표 글자수를 설정하면 진행률이 표시됩니다</div>
          </div>
        )}
        {stats.lastActivity && (
...
export default function DashboardPage() {
  const { notes, references, projects, setCurrentProject, isLoading } = useData();
  const [recentItems, setRecentItems] = useState([]);
  const [showNewNoteModal, setShowNewNoteModal] = useState(false); // Unused in this file, but kept for completeness
  ...
  const handleSelectRecentItem = (item, type) => {
    if (type === 'note') {
      ...
      window.location.href = createPageUrl('Workspace') + `?referenceId=${item.id}`;
    }
  };
  ...
  const getProjectStats = (projectId) => {
    ...
    const lastActivity = projectNotes.length > 0 
      ? Math.max(...projectNotes.map(note => new Date(note.updated_date).getTime()))
      : null;
  ...
  const todayNotes = notes.filter(note => isToday(new Date(note.updated_date))).length;
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">스크립트 스튜디오</h1>
              <p className="text-slate-600 mt-1">창작 프로젝트를 체계적으로 관리하세요</p>
            </div>
            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={handleGoToWorkspace}>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={handleGoToWorkspace}>
                  <Plus className="h-4 w-4 mr-2" />
                  새 프로젝트
                </Button>
              </div>
              ...
              {projects.length === 0 ? (
                <Card className="text-center py-12">
                  <CardContent>
                    ...
                    <h3 className="text-lg font-medium text-slate-700 mb-2">프로젝트를 시작해보세요</h3>
                    <p className="text-slate-500 mb-6">첫 번째 창작 프로젝트를 만들어 체계적으로 관리해보세요.</p>
                    <Button onClick={handleGoToWorkspace}>
                      <Plus className="h-4 w-4 mr-2" />
                      새 프로젝트 만들기
                    </Button>
                ...
                <CardContent className="p-4">
                  <div className="space-y-3">
                    ...
                    <Button 
                      variant="ghost" 
                      className="w-full justify-start h-10" 
                      onClick={() => window.location.href = createPageUrl('Workspace') + '?view=daily'}
                    >
