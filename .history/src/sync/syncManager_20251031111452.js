...
class SyncManager {
  ...
  startAutoSync(intervalMs = 30000) {
    console.log('🏠 로컬 전용 모드 - 자동 동기화 비활성화');
    // 로컬 전용 모드에서는 동기화하지 않음
    return;
  ...
  async pushToServer() {
    ...
    for (const item of queue) {
      try {
        ...
        switch (item.action) {
          case "create":
            await this.apiClient.post(item.endpoint, item.data);
            success = true;
            break;

          case "update":
            await this.apiClient.put(item.endpoint, item.data);
            success = true;
            break;

          case "delete":
            await this.apiClient.delete(item.endpoint);
            success = true;
            break;
        ...
        if (success) {
          ...
          if (item.action !== "delete" && item.data?.id) {
            const localItem = await localDB.get(item.store, item.data.id);
            if (localItem) {
              await localDB.put(item.store, {
                ...localItem,
                sync_status: "synced",
              });
            }
          ...
          console.log(`✅ ${item.action} 성공: ${item.endpoint}`);
        }
      } catch (error) {
