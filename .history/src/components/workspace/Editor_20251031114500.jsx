...
export default function Editor({ note, onNavigateToNote, isReadMode: externalReadMode, onEnterTranslationMode }) {
    ...
    useEffect(() => {
        if (quillRef.current && !isReadMode) {
    ...
    const handleSelectSuggestion = (selectedTitle) => {
        ...
        quill.deleteText(startPos - 2, query.length + 2); 
        quill.insertText(startPos - 2, `[[${selectedTitle}]]`);
        quill.setSelection(startPos - 2 + selectedTitle.length + 4); 
        closeSuggestionPopover();
    };
    ...
    useEffect(() => {
      ...
      const updateCurrentLine = () => {
        ...
        const allLines = editor.querySelectorAll('.ql-editor > *');
        allLines.forEach(el => el.classList.remove('current-line'));
        ...
        setCurrentLine(selection && selection.length === 0 ? selection.index : null);
      };
    ...
    }, [isReadMode, enableLineHighlight]);
    ...
    useEffect(() => {
      ...
      allLines.forEach((el, index) => {
        ...
        if (enableZebraStripes) {
          if (index % 2 === 0) {
            el.classList.add('zebra-even');
          } else {
            el.classList.add('zebra-odd');
          }
        }
      });
    }, [content, enableZebraStripes, isReadMode]);
    ...
    const handleAddFootnote = () => {
        ...
        if (quillRef.current) {
            ...
            quill.insertText(range.index, `[${newId}]`);
        }
    };
    ...
    const handleReferenceInsert = (data) => {
        if (quillRef.current) {
            const quill = quillRef.current.getEditor();
            quill.insertText(quill.getSelection(true).index, data.citationText);
        }
        ...
        if (suggestionState.isOpen && suggestionState.suggestions.length > 0) {
            if (event.key === 'ArrowDown') {
                ...
                if (suggestionState.suggestions.length > 0) {
                    const selectedNote = suggestionState.suggestions[suggestionState.selectedIndex];
    ...
    const quillModules = {
        toolbar: [[{ 'header': [1, 2, 3, false] }],['bold', 'italic', 'underline','strike', 'blockquote', 'code-block'],[{'list': 'ordered'}, {'list': 'bullet'}],['link'],['clean']],
        clipboard: {
            matchers: []
        }
    };
    ...
    return (
        <div className="h-full flex flex-col bg-white relative">
                    ...
                    <div ref={editorContainerRef} className="flex-1 overflow-y-auto" style={{ height: 0 }}>
                        {isReadMode ? (
                            ...
                            <ReactQuill 
                                ...
                                className="h-full editor-no-spellcheck"
            ...
            <style>{`
                ...
                .ql-editor { font-size: 1.1rem; line-height: 1.8; padding: 2rem 4rem; }
                .ql-toolbar.ql-snow { border: none; border-bottom: 1px solid #f1f5f9; position: sticky; top: 0; background: white; z-index: 10; }
                .prose { font-size: 1.1rem; line-height: 1.8; padding: 2rem 4rem; }
                ...
                .editor-no-spellcheck .ql-editor {
                ...
                .ql-editor > .current-line.zebra-even,
                .ql-editor > .current-line.zebra-odd {
                  background-color: rgba(59, 130, 246, 0.08) !important;
                }
