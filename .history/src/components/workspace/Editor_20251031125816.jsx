...
export default function Editor({
  ...
  const handleEditorChange = useCallback(
    (html) => {
      console.log("🔄 handleEditorChange 호출됨:", html.substring(0, 50));
      setContent(html);
  ...
  const handleAddFootnote = () => {
    console.log("📌 각주 추가 버튼 클릭됨");
    const newId =
      footnotes.length > 0 ? Math.max(...footnotes.map((f) => f.id)) + 1 : 1;
    console.log("📌 새 각주 ID:", newId);
    setFootnotes([...footnotes, { id: newId, content: "" }]);
    if (editorRef.current) {
      console.log("📌 editorRef 존재함");
      const selection = editorRef.current.getSelection();
      console.log("📌 현재 선택 위치:", selection);
      if (selection) {
        editorRef.current.insertText(`[${newId}]`, selection.index);
        console.log("✅ 각주 번호 삽입 완료:", `[${newId}]`);
      } else {
        console.warn("⚠️ 선택 위치가 없습니다");
      }
    } else {
      console.error("❌ editorRef가 null입니다");
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-white relative">
      ...
      <WritingStats
        content={content}
        isVisible={showWritingStats}
        onToggle={() => setShowWritingStats(!showWritingStats)}
      />
