...
export default function NewNoteModal({ isOpen, onClose, folderId, onNoteCreated }) {
    ...
    const handleCreate = async () => {
        ...
        try {
            console.log('📝 Note.create 호출:', { title, content: content.substring(0, 50), project_id: currentProject?.id, folder_id: folderId });
                                ...
                                <ScrollArea className="h-48 border rounded-md p-4">
                                    <div className="space-y-2">
                                        <div className="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-50">
                                            ...
                                            <Label htmlFor="blank" className="font-medium flex-1 cursor-pointer">
                                        ...
                                        {templates.map(template => (
                                            <div key={template.id} className="flex items-start space-x-2 p-2 rounded-md hover:bg-gray-50">
                                                <RadioGroupItem value={template.id} id={template.id} className="mt-1" />
                                                <Label htmlFor={template.id} className="font-normal flex-1 cursor-pointer">
