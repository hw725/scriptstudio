...
export default function Editor({
  ...
  const handleEditorChange = useCallback(
    (html) => {
      setContent(html);
      setHasUnsavedChanges(true); // 변경사항 플래그 설정
  ...
  useEffect(() => {
    if (note?.id && note.id !== currentNoteId) {
      if (currentNoteId !== null && hasUnsavedChanges && !justSaved) {
        if (
          window.confirm(
            "저장되지 않은 변경사항이 있습니다. 저장하고 이동하시겠습니까?"
          )
        ) {
          handleSave(false);
        } else {
  ...
  useEffect(() => {
    ...
    const footnotesChanged =
      JSON.stringify(footnotes) !== JSON.stringify(originalData.footnotes);
  ...
  const loadVersions = useCallback(async () => {
    if (!note?.id) return;
    const fetchedVersions = await NoteVersion.filter(
      { note_id: note.id },
      "-created_date",
      50
    );
  ...
  const handleRestoreVersion = async (version) => {
    if (
      !note?.id ||
      !window.confirm(
        "이 버전으로 복원하시겠습니까? 현재 내용은 덮어쓰여집니다."
      )
    )
      return;
  ...
  const projectNotes = currentProject
    ? allNotes.filter((n) => n.project_id === currentProject.id)
    : allNotes;
  const projectChars = projectNotes.reduce(
    (sum, n) => sum + getCharCount(n.content),
    0
  );
  const projectTarget = projectNotes.reduce(
    (sum, n) => sum + (n.target_words || 0),
    0
  );
    ...
    <div className="h-full flex flex-col bg-white relative">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    if (window.confirm("변경사항을 버리시겠습니까?")) {
                      setShowDataLossWarning(false);
                      setHasUnsavedChanges(false);
                      setCurrentNoteId(null);
                    }
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <DropdownMenu>
            ...
            >
              ...
              {/* 편집 보조 섹션 */}
              <DropdownMenuSeparator />
              <div className="px-2 py-1 text-[11px] font-semibold text-slate-500">편집 보조</div>
              <DropdownMenuItem
              ...
              {/* 행 스타일 섹션 */}
              <div className="px-2 pt-1 pb-0.5 text-[11px] font-semibold text-slate-500">행 스타일</div>
              <DropdownMenuItem
                onClick={() => setEnableLineHighlight(!enableLineHighlight)}
                className="flex items-center justify-between"
              >
                <div className="flex items-center gap-2">
                  <input type="checkbox" readOnly checked={!!enableLineHighlight} className="h-3.5 w-3.5 accent-slate-600" />
                  <span>현재 줄 강조</span>
                </div>
              </DropdownMenuItem>
              <DropdownMenuItem
              ...
              >
                <div className="flex items-center gap-2">
                  <input type="checkbox" readOnly checked={!!enableZebraStripes} className="h-3.5 w-3.5 accent-slate-600" />
                  <span>줄무늬</span>
                </div>
              </DropdownMenuItem>
              ...
              {/* 패널/내보내기 섹션 */}
              <DropdownMenuSeparator />
              <div className="px-2 py-1 text-[11px] font-semibold text-slate-500">패널 / 내보내기</div>
      ...
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between gap-4">
          ...
          {/* 행 스타일 관련 버튼은 상단 더보기 메뉴로 이동하여 공간 절약 */}
        </div>
      </div>
            ...
            {isReadMode ? (
              <div
                ...
                className={`p-8 prose prose-slate max-w-none dark:prose-invert ${
                  useSerifFont ? "font-serif-readmode" : "font-sans-readmode"
                }`}
              >
                <div
                  dangerouslySetInnerHTML={{
                    __html: renderLinksInContent(
                      content,
                      allNotes,
                      currentProject?.id
                    ),
      ...
      {!onEnterTranslationMode && (
        <PomoFlowPanel
          note={note}
          isVisible={showPomoFlowPanel}
          onToggle={() => setShowPomoFlowPanel(!showPomoFlowPanel)}
        />
      )}
