...
import { WelcomeScreen, LoadingScreen } from '@/components/workspace/Placeholders';
...
export default function WorkspacePage() {
  ...
  const { notes, folders, references, projects, currentProject, setCurrentProject, isLoading, refetchData, allNotes } = useData();

  const handleSelectNote = useCallback((noteId, isInitialLoad = false) => {
    ...
    if (targetNote) {
      ...
      if (targetNote.project_id !== currentProject?.id) {
        if (targetNote.project_id) {
          const targetProject = projects.find(p => p.id === targetNote.project_id);
          if (targetProject) {
  ...
  useEffect(() => {
    ...
    if (selectedReferenceId && !references.some(r => r.id === selectedReferenceId)) {
      setSelectedReferenceId(null);
    }
  ...
  // 번역 모드 진입
  const handleEnterTranslationMode = useCallback((noteId) => {
    // 현재 상태 저장
    ...
    <div className="flex h-full w-full bg-slate-50">
      {!isTranslationMode && ( // Sidebar and its header are hidden in translation mode
        <div className="flex flex-col h-full shrink-0">
          {!sidebarCollapsed && (
            <div 
              className="h-12 px-4 py-3 border-b border-r border-slate-200 bg-white flex items-center gap-2"
              style={{ width: `${sidebarWidth}px` }}
            >
              <Link to={createPageUrl('Dashboard')}>
                <Button variant="ghost" size="icon" title="대시보드로 이동" className="h-6 w-6">
