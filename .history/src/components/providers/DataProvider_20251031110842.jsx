...
export const DataProvider = ({ children }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [projectsData, notesData, foldersData, referencesData] = await Promise.all([
      ...
      if (currentProject && !safeProjects.some(p => p.id === currentProject.id)) {
      ...
      if (err.message && (err.message.includes('429') || err.message.includes('Rate limit exceeded'))) {
  ...
  const updateReferenceInState = (updatedReference) => {
    setReferences(prev => prev.map(r => r.id === updatedReference.id ? { ...r, ...updatedReference } : r));
  };
