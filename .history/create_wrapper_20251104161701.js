...
const code = `import { localDB } from "../db/localDB";
...
class OfflineEntityWrapper {
  ...
  async create(data) {
    ...
    if (navigator.onLine) {
      try {
        const serverData = await this.apiEntity.create(data);
  ...
  async update(id, data) {
    const localData = { ...data, id, sync_status: "pending" };
    await localDB.put(this.storeName, localData);
    if (navigator.onLine) {
      try {
...
fs.writeFileSync('src/api/offlineWrapper.js', code, 'utf-8');
console.log('✅ offlineWrapper.js created successfully');
